...
from TotumApp.models import Tenant, TotumUser
...
class TenantViewSet(viewsets.ModelViewSet):
    ...
    def add_users(self, request: Request, uuid=None):
        ...
        serializer.is_valid(raise_exception=True)
        tenant.users.add(*TotumUser.objects.filter(uuid__in=serializer.validated_data['uuids']))
        tenant.save()
    ...
    def remove_users(self, request: Request, uuid=None):
        ...
        serializer = UserUuidListSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        tenant.users.remove(*TotumUser.objects.filter(uuid__in=serializer.validated_data['uuids']))
        tenant.save()
        return Response(SimpleResponseSerializer({"message": "Success"}).data)
